...
public final class PluginsRegistry {
  ...
  public func register(plugin: ReactNativeVideoPluginSpec) {
      ...
      if hasPlugin(plugin: plugin) {
        print(
          "[UnifiedPlayer] Plugin \(plugin.name) (ID: \(plugin.id)) is already registered - overwriting."
        )
      } else {
        print(
          "[UnifiedPlayer] Registering plugin \(plugin.name) (ID: \(plugin.id))."
        )
      }
    #endif
  ...
  public func unregister(plugin: ReactNativeVideoPluginSpec) {
      ...
      if !hasPlugin(plugin: plugin) {
        print(
          "[UnifiedPlayer] Plugin \(plugin.name) (ID: \(plugin.id)) is not registered - skipping."
        )
      } else {
        print("[UnifiedPlayer] Unregistering plugin \(plugin.name) (ID: \(plugin.id)).")
      }
    #endif
